...
exports.tryToAuthorize = async (basePath) => {
  ...
  await hubspotLoginPage.login(
    process.env.HUBSPOT_AUTHORIZATION_EMAIL,
    process.env.HUBSPOT_AUTHORIZATION_PASSWORD
  );
  const hubspotAccountsListPage = await hubspotAccountsListFactory(
    driver,
    process.env.HUBSPOT_ACCOUNT_NAME
  );
